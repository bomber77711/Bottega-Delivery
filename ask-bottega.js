// Only allow requests from our own domains (the widget is same-origin,
// so CORS here is just a hard lock against third-party sites using our API key).
...
// For stronger guarantees use Upstash/Vercel KV.
const RATE_LIMIT = { windowMs: 60_000, max: 10 };
const hits = new Map();
function rateLimited(ip) {
  const now = Date.now();
  const entry = hits.get(ip) || { count: 0, start: now };
  if (now - entry.start > RATE_LIMIT.windowMs) { entry.count = 0; entry.start = now; }
  entry.count += 1;
  hits.set(ip, entry);
